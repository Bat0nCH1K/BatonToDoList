...
function selectFolder(id) {
    currentFolder = id; renderFolders(); renderTasks();
    document.getElementById('sidebar').classList.remove('active');
    document.getElementById('sidebarOverlay').classList.remove('active');
    const folder = folders.find(f => f.id === id);
    document.getElementById('currentFolderName').textContent = (folder && folder.id === 'all') ? t('allTasks') : folder?.name || t('allTasks');
}
...
function renderTasks() {
    ...
    list.innerHTML = filtered.map(task => {
        const done = task.progress || 0, total = task.repeat || 1;
        ...
        return `
            <div class="task-item ${cls}" style="border-left:4px solid ${task.color || '#e94560'}">
                ...
                <div class="task-info" onclick="openTask(${task.id})">
                    ...
                    <div class="task-meta">${task.deadline ? '📅 ' + formatDate(task.deadline) : ''}${overdue ? ' ⚠️' : ''}</div>
...
function formatDate(d) { const dt = new Date(d); return dt.toLocaleDateString('ru-RU', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' }); }
...
function openTask(id) {
    ...
    if (task.image) { document.getElementById('imagePreview').src = task.image; document.getElementById('imagePreview').style.display = 'block'; }
...
document.getElementById('saveTaskBtn').addEventListener('click', async () => {
    ...
    if (imgFile) {
        imageData = await new Promise(r => { const fr = new FileReader(); fr.onload = () => r(fr.result); fr.readAsDataURL(imgFile); });
    }
    const data = {
        name, color: document.getElementById('taskColor').value,
    ...
    if (editingTaskId) { const i = tasks.findIndex(t => t.id === editingTaskId); if (i !== -1) tasks[i] = { ...tasks[i], ...data }; }
    else { tasks.push({ id: Date.now(), ...data }); }
    save(); renderTasks(); document.getElementById('taskModal').classList.remove('active');
});

document.getElementById('cancelTaskBtn').addEventListener('click', () => document.getElementById('taskModal').classList.remove('active'));
document.getElementById('taskDeadline').addEventListener('change', function() { document.getElementById('previewDeadline').textContent = this.value ? formatDate(this.value) : ''; });
...
document.getElementById('taskImage').addEventListener('change', function() {
    ...
    if (f) { const fr = new FileReader(); fr.onload = e => { document.getElementById('imagePreview').src = e.target.result; document.getElementById('imagePreview').style.display = 'block'; }; fr.readAsDataURL(f); }
});
...
document.getElementById('calendarBtn').addEventListener('click', () => {
    const now = new Date(); calYear = now.getFullYear(); calMonth = now.getMonth(); selectedDate = null;
    renderCalendar(); document.getElementById('calendarScreen').classList.add('active');
});
...
function renderCalendar() {
    ...
    const grid = document.getElementById('calendarGrid'), firstDay = new Date(calYear, calMonth, 1).getDay() || 7;
    const days = new Date(calYear, calMonth + 1, 0).getDate(), today = new Date();
    let html = '';
    for (let i = 1; i < firstDay; i++) html += '<div class="calendar-cell"></div>';
    for (let d = 1; d <= days; d++) {
        const ds = `${calYear}-${String(calMonth+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
        const has = tasks.some(t => t.deadline && t.deadline.startsWith(ds));
        const isToday = today.getFullYear() === calYear && today.getMonth() === calMonth && today.getDate() === d;
        html += `<div class="calendar-cell ${has ? 'has-task' : ''} ${isToday ? 'today' : ''} ${selectedDate === ds ? 'selected' : ''}" onclick="selectCalendarDate('${ds}')">${d}</div>`;
    }
    grid.innerHTML = html;
    const tld = document.getElementById('calendarTaskList');
    if (selectedDate) {
        const dt = tasks.filter(t => t.deadline && t.deadline.startsWith(selectedDate)).sort((a,b) => (a.deadline||'').localeCompare(b.deadline||''));
        tld.innerHTML = dt.length === 0 ? `<p style="color:var(--text-secondary);text-align:center;">${t('noTasksOnDate')}</p>` :
            `<p style="font-weight:600;margin-bottom:8px;">${t('tasksOnDate')} ${formatDate(selectedDate)}</p>` + dt.map(task => `
                <div class="task-item" style="border-left:4px solid ${task.color||'#e94560'};margin-bottom:6px;" onclick="openTaskFromCalendar(${task.id})">
                    <div class="task-info"><div class="task-name">${escapeHtml(task.name)}</div><div class="task-meta">${formatDate(task.deadline)}</div></div>
                </div>`).join('');
        tld.innerHTML += `<button class="btn" style="margin-top:10px;width:100%;" onclick="addTaskForDate('${selectedDate}')">+ ${t('createTask')}</button>`;
    } else { tld.innerHTML = ''; }
}
function selectCalendarDate(d) { selectedDate = d; renderCalendar(); }
function addTaskForDate(d) { document.getElementById('calendarScreen').classList.remove('active'); document.getElementById('taskDeadline').value = d + 'T12:00'; document.getElementById('previewDeadline').textContent = formatDate(d+'T12:00'); editingTaskId = null; document.getElementById('modalTitle').textContent = t('newTask'); document.getElementById('taskNameInput').value = ''; document.getElementById('taskColor').value = '#e94560'; document.getElementById('taskRepeat').value = '1'; document.getElementById('taskImage').value = ''; document.getElementById('imagePreview').style.display = 'none'; document.getElementById('colorPreview').style.background = '#e94560'; document.getElementById('taskModal').classList.add('active'); }
...
function renderStats() {
    ...
    document.getElementById('statDone').parentElement.innerHTML = t('done') + ': <span id="statDone">' + done + '</span>';
    document.getElementById('statActive').parentElement.innerHTML = t('active') + ': <span id="statActive">' + Math.max(0, active) + '</span>';
    document.getElementById('statOverdue').parentElement.innerHTML = t('overdue') + ': <span id="statOverdue">' + overdue + '</span>';
    const canvas = document.getElementById('statsChart'), ctx = canvas.getContext('2d');
    ctx.clearRect(0, 0, 220, 220);
    const total = tasks.length || 1, colors = ['#4caf84','#e94560','#ff9800'], values = [done, Math.max(0,active), overdue];
    let start = -Math.PI/2;
    values.forEach((v,i) => { const slice = (v/total)*Math.PI*2; ctx.beginPath(); ctx.moveTo(110,110); ctx.arc(110,110,90,start,start+slice); ctx.fillStyle = colors[i]; ctx.fill(); start += slice; });
    ctx.beginPath(); ctx.arc(110,110,45,0,Math.PI*2); ctx.fillStyle = getComputedStyle(document.body).getPropertyValue('--bg').trim(); ctx.fill();
}
...
function translateUI() {
    ...
    document.getElementById('saveTaskBtn').textContent = t('save');
    document.getElementById('cancelTaskBtn').textContent = t('cancel');
    document.getElementById('saveFolderBtn').textContent = t('save');
    document.getElementById('cancelFolderBtn').textContent = t('cancel');
    document.getElementById('labelDeadline').innerHTML = t('deadline') + ': <input type="datetime-local" id="taskDeadline">';
    document.getElementById('labelColor').innerHTML = t('color') + ': <input type="color" id="taskColor" value="#e94560"><span class="color-preview" id="colorPreview" style="background:#e94560"></span>';
    document.getElementById('labelRepeat').innerHTML = t('repeat') + ': <input type="number" id="taskRepeat" value="1" min="1" max="99">';
    document.getElementById('labelImage').innerHTML = t('image') + ': <input type="file" id="taskImage" accept="image/*">';
    document.getElementById('taskNameInput').placeholder = t('taskName');
    ...
    if (currentFolder === 'all') document.getElementById('currentFolderName').textContent = t('allTasks');
    renderFolders();
}
